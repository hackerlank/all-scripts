...
source $(dirname $0)/wj_proxy_hosts.txt
...
for l in ${hosts[@]}; do
	...
	if [ -z "$pid" ]; then
		...
		${AUTOSSH} -M ${monitor_port} -Nnf -D ${proxy_port} ${host_user}@${host_ip} -p${host_port} -i /home/ec2-user/.ssh/sg_proxy.pem
		sleep 5
	fi
	...
	printf "%-45s%10s%10s\n" ${prefix}_${host_ip} ${time} average_time:${average_time}
	#echo $average_time
	if [ $(echo $average_time | awk '{if($1>7){print 1;}else{print 0}}') -eq 1 ] || [ $r -ne 0 ]; then
		echo "disable server ${backend_server}/${prefix}_${host_ip}" | socat stdio ${haproxy_sock} >/dev/null 2>&1
		echo "disable server ${backend_test_server}/${prefix}_${host_ip}" | socat stdio ${haproxy_sock} >/dev/null 2>&1
	else
