...
#SVN地址信息
#SVN_ADDR='https://svn.pupugao.com/svn/src/web/trunk/mps'
SVN_ADDR='https://svn.digi800.com/svn/src/web/trunk/mps'
...
/usr/bin/ssh -p38022 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -oConnectTimeout=15 syncer@101.231.200.238 "echo $VERSION > /tmp/deploymps"
...
if [ $? != 0 ];then
    ...
    echo "rsync code from 192.168.0.230 to 192.168.1.51  OK ^_^ !!!"
fi

count=0
timeout=30
while [ $count -lt $timeout ];do
return=`sudo  /usr/bin/ssh  -p38022 -oConnectTimeout=15 syncer@101.231.200.238 "cat /tmp/successmps"`
  if [ $return == "OK" ];then
    /usr/bin/ssh -p38022 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -oConnectTimeout=15 syncer@101.231.200.238 "echo 0 > /tmp/successmps" 
