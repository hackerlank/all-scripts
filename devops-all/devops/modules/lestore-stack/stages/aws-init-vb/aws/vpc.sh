...
username1_policy_name1=vb-up-cf
username1_policy_name2=vb-up-ec2
username1_policy_name3=vb-up-rds
username1_policy_name4=vb-up-s3
...
# for s3
bucket_name_img=vbimg
bucket_name_dbbak=vbdbbak
# for cloudfront
...
sg_vbcachesg_id=$(aws ec2 create-security-group --region=$region --output text --group-name $sg_vbcachesg --description "created by script" --vpc-id $vpc_id|awk '{print $1}'|head -1)
sg_vbdbsg_id=$(aws ec2 create-security-group --region=$region --output text --group-name $sg_vbdbsg --description "created by script" --vpc-id $vpc_id|awk '{print $1}'|head -1)
sg_vbelbsg_id=$(aws ec2 create-security-group --region=$region --output text --group-name $sg_vbelbsg --description "created by script" --vpc-id $vpc_id|awk '{print $1}'|head -1)
# check
...
if [ -z $sg_vbcachesg_id ]; then
	sg_vbcachesg_id=$(echo "$sg_list"|grep 'SECURITYGROUPS'|awk -F "\t" '{if($4=="'$sg_vbcachesg'"){print $3}}')
fi
if [ -z $sg_vbdbsg_id ]; then
	sg_vbdbsg_id=$(echo "$sg_list"|grep 'SECURITYGROUPS'|awk -F "\t" '{if($4=="'$sg_vbdbsg'"){print $3}}')
fi
if [ -z $sg_vbelbsg_id ]; then
	sg_vbelbsg_id=$(echo "$sg_list"|grep 'SECURITYGROUPS'|awk -F "\t" '{if($4=="'$sg_vbelbsg'"){print $3}}')
fi
...
# lestore-production
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_lestore_production_id --protocol tcp --port 80 --source-group $sg_vbelbsg_id
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_lestore_production_id --protocol tcp --port 443 --source-group $sg_vbelbsg_id
...
# vbelbsg
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_vbelbsg_id --protocol tcp --port 80 --cidr 0.0.0.0/0
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_vbelbsg_id --protocol tcp --port 443 --cidr 0.0.0.0/0
# vbdbsg
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_vbdbsg_id --protocol tcp --port 3306 --source-group $sg_lestore_production_id
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_vbdbsg_id --protocol tcp --port 3306 --source-group $sg_lestore_devops_deployer_id
# vbcachesg
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_vbcachesg_id --protocol tcp --port 11211 --source-group $sg_lestore_production_id
...
elb_pre_id=$(aws elb create-load-balancer --region $region --output text --load-balancer-name $elb_pre \
	...
	--security-group $sg_vbelbsg_id --availability-zones $elb_availability_zones)
...
aws elasticache create-cache-cluster --region $region --cache-cluster-id $cache_session_id --num-cache-nodes $cache_session_num_nodes \
	...
	--security-group-ids $sg_vbcachesg_id
...
#	--vpc-security-group-ids $sg_vbdbsg_id --multi-az --db-parameter-group-name $dbmysqlpg
# create rds slave
