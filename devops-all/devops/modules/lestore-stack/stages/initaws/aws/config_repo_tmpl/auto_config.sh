...
__SEARCH_MAIN__=$(aws ec2 describe-instances --region $__AWS_REGION__ --output text \
	...
	--filters "Name=tag-value,Values=lestore-stage-azazie-search-prod-0" | grep -E "^INSTANCE" | awk '{print $15}'|grep amazonaws)
...
__SEARCH_BACK__=$(aws ec2 describe-instances --region $__AWS_REGION__ --output text \
	--filters "Name=tag-key,Values=aws:cloudformation:stack-name" \
	--filters "Name=tag-value,Values=lestore-stage-azazie-search-prod-2" | grep -E "^INSTANCE" | awk '{print $15}'|grep amazonaws)
