...
username1=jjshouse_vm_cms-a
username1_policy_name1=zz-up-cf-a
username1_policy_name2=zz-up-ec2-a
username1_policy_name3=zz-up-rds-a
username1_policy_name4=zz-up-s3-a
username2=jjshouse_vm_normal-a
username2_policy_name1=jjshouse_vm-send-email-a
# for security group
...
is_exists=$(echo "$vpcs"|awk '{if ($1=="TAGS" && $2=="Name" && $3=="'"$vpc_tag_name"'"){print $0}}'|wc -l)
if [ $is_exists -lt 1 ]; then
	vpc=$(aws ec2 create-vpc --region=$region --output text --cidr-block $CIDR)
	vpc_id=$(echo $vpc|awk '{print $6}')
...
sg_zzcachesg_id=$(aws ec2 create-security-group --region=$region --output text --group-name $sg_zzcachesg --description "created by script" --vpc-id $vpc_id|awk '{print $1}'|head -1)
sg_zzdbsg_id=$(aws ec2 create-security-group --region=$region --output text --group-name $sg_zzdbsg --description "created by script" --vpc-id $vpc_id|awk '{print $1}'|head -1)
sg_zzelbsg_id=$(aws ec2 create-security-group --region=$region --output text --group-name $sg_zzelbsg --description "created by script" --vpc-id $vpc_id|awk '{print $1}'|head -1)
# check
...
if [ -z $sg_zzcachesg_id ]; then
	sg_zzcachesg_id=$(echo "$sg_list"|grep 'SECURITYGROUPS'|awk -F "\t" '{if($4=="'$sg_zzcachesg'"){print $3}}')
fi
if [ -z $sg_zzdbsg_id ]; then
	sg_zzdbsg_id=$(echo "$sg_list"|grep 'SECURITYGROUPS'|awk -F "\t" '{if($4=="'$sg_zzdbsg'"){print $3}}')
fi
if [ -z $sg_zzelbsg_id ]; then
	sg_zzelbsg_id=$(echo "$sg_list"|grep 'SECURITYGROUPS'|awk -F "\t" '{if($4=="'$sg_zzelbsg'"){print $3}}')
fi
...
# lestore-production
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_lestore_production_id --protocol tcp --port 80 --source-group $sg_zzelbsg_id
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_lestore_production_id --protocol tcp --port 443 --source-group $sg_zzelbsg_id
...
# zzelbsg
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_zzelbsg_id --protocol tcp --port 80 --cidr 0.0.0.0/0
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_zzelbsg_id --protocol tcp --port 443 --cidr 0.0.0.0/0
# zzdbsg
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_zzdbsg_id --protocol tcp --port 3306 --source-group $sg_lestore_production_id
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_zzdbsg_id --protocol tcp --port 3306 --source-group $sg_lestore_devops_deployer_id
# zzcachesg
aws ec2 authorize-security-group-ingress --region $region --group-id $sg_zzcachesg_id --protocol tcp --port 11211 --source-group $sg_lestore_production_id
...
elb_pre_id=$(aws elb create-load-balancer --region $region --output text --load-balancer-name $elb_pre \
	...
	--security-group $sg_zzelbsg_id --availability-zones $elb_availability_zones)
...
aws elasticache create-cache-cluster --region $region --cache-cluster-id $cache_session_id --num-cache-nodes $cache_session_num_nodes \
	...
	--security-group-ids $sg_zzcachesg_id
...
#	--vpc-security-group-ids $sg_zzdbsg_id --multi-az --db-parameter-group-name $dbmysqlpg
# create rds slave
