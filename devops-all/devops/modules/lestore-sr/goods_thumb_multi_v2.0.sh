...
SELECT gt.goods_thumb_id, gt.goods_id, gt.img_id, gt.thumb_name, gt.width, gt.height, gt.watermark, gg.img_url, img_original, gg.img_type
FROM goods_thumb gt LEFT JOIN goods_gallery gg on gt.img_id = gg.img_id
WHERE gt.has_done = 0 AND gg.img_type != 'banner' AND goods_thumb_id % $thread_count = $thread_no
$condition
...
AND gt.has_done = 0 AND gg.img_type != 'banner'
SQL_END
    while read row; do
        if [ "$iii" = "" ]; then
                iii=0
...
#fi
#process_by_goods_thumb_id 9120179
