...
then
    ...
    /usr/bin/rsync -vzrtopg -e "ssh -p32200 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null" --progress --delete syncer@$local_server:/opt/data1/code/$name.tar.gz /tmp
    ...
    /usr/bin/rsync -vzrtopg --progress --delete --exclude "/config/config.inc.php" /tmp/$name/ /var/www/http/$name
    echo
