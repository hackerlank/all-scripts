...
lebbay::os::cronjob{ "$stagename.clean_cronjob_log" :
        script  => false,
        command => 'find /var/log/top.*.log -mtime +7|xargs -I {} rm -fv {}',
        user    => 'root',
        minute  => 0,
        hour  => 3;
}
...
lebbay::os::cronjob{ "$stagename.goods_thumb" :
        script  => false,
        command => '/var/job/goods_thumb_m.sh',
        user    => 'root';
}

#lebbay::os::cronjob{ "$stagename.goods_banner" :
#        script  => false,
#		command => '/var/job/goods_banner.sh',
#        user    => 'root';
#}
